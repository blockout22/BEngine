...
public class OldCamera {
	...
	private static ArrayList<Vector3f> startPositions = new ArrayList<Vector3f>();
	private static ArrayList<Vector3f> endPositions = new ArrayList<Vector3f>();
	...
	private boolean defaultFloorCollision = false;
	...
	public OldCamera(float fov, float aspectRatio, float z_near, float z_far) {
		...
		// floor collision
		if (defaultFloorCollision) {
			addCollisionBox(-1000000, 0, -1000000, 1000000000);
		}
	}
	
	public void enableDefaultFloorCollision()
	{
		defaultFloorCollision = true;
	}
			...
			for (Vector3f end : endPositions) {
				...
				if (xStart && xEnd && yStart && yEnd && zStart && zEnd) {
					...
					if (yfpos < yspos) {
						...
						canMoveYPlus = false;
						// onGround = true;
						// cameraPos.y = cameraPos.y - 0.001f;
					}
	...
	protected static void addCollisionBox1(float x, float y, float z, int size) {
		float xStart = x;
		float yStart = y;
		float zStart = z;
		startPositions.add(new Vector3f(xStart, yStart, zStart));
